...
# dont know if moviepi editor is a necessity or not. Will find out when it runs
...
def run(txtFile,audioFile,forwardGoPro,faceGoPro):
    ...
    with open(timesFile) as csv_file:
            ...
            for row in csv_reader:
                if (row[0] == 'Video started at'):
                    times.append(row)
                elif (row[0] == 'Forward facing video at'):
                    times.append(row)
                elif (row[0] == 'Audio started at'): #This one slightly problematic as the txt file doesnt have the audio start tine anymore
                    times.append(row)
    ...
    for x in times:
        ...
        elif x[0] == 'Forward facing video at':
    ...
    if all_reference_times_exist:
        ...
        face_vid_path = list_of_files[2] # show an "Open" dialog box and return the path to the selected file
        forward_vid_path = list_of_files[1] # show an "Open" dialog box and return the path to the selected file
        audio_file_path = list_of_files[0] # show an "Open" dialog box and return the path to the selected file
        ...
        if (unix_times_used_to_cut_vid_or_audio[0][0] == "Face facing"):
            shell_command_1 = "ffmpeg -ss " + cutoff_1_string + " -i input/" + face_vid + " -c copy output/" + face_vid[:-4] + "Synced.mp4"
        elif (unix_times_used_to_cut_vid_or_audio[0][0] == "Forward facing"):
            shell_command_1 = "ffmpeg -ss " + cutoff_1_string + " -i input/" + forward_vid + " -c copy output/" + forward_vid[:-4]  + "Synced.mp4"
        elif (unix_times_used_to_cut_vid_or_audio[0][0] == "audio time"):
            shell_command_1 = "ffmpeg -ss " + cutoff_1_string + " -i input/" + audio + " -c copy output/" + audio[:-4]  + "Synced.mp4"

        if (unix_times_used_to_cut_vid_or_audio[1][0] == "Face facing"):
            shell_command_2 = "ffmpeg -ss " + cutoff_2_string + " -i input/" + face_vid + " -c copy output/" + face_vid[:-4]  + "Synced.mp4"
        elif (unix_times_used_to_cut_vid_or_audio[1][0] == "Forward facing"):
            shell_command_2 = "ffmpeg -ss " + cutoff_2_string + " -i input/" + forward_vid + " -c copy output/" + forward_vid[:-4]  + "Synced.mp4"
        elif (unix_times_used_to_cut_vid_or_audio[1][0] == "audio time"):
            shell_command_2 = "ffmpeg -ss " + cutoff_2_string + " -i input/" + audio + " -c copy output/" + audio[:-4]  + "Synced.wav"
